...
const usersBelow25 = usersExercise4.filter(user => {
  if (typeof user.age === 'number') {
    ...
    const textTypeNumber = parseInt(user.age);
    if (typeof textTypeNumber === 'number') {
      return user.age < 25;
    }
    return false;
  }
...
const outOfStock = productsExercise6.filter(products => {
  if (products.quantity === 0) {
    return console.log(`${products.brand} ${products.name}`,"is out of stock.");
  } else {
    return false;
  }
});
...
const gradeHigherThanC = studentsExercise9.filter(student => {
  const studentGrades = gradeValues[student.grade];
  return studentGrades >= gradeValues["C"];
});
console.log(gradeHigherThanC);
// Exercise 10:
