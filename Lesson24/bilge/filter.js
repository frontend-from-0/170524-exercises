...
const peopleUnderAge25 = usersExercise4.filter(user => user.age < 25);
console.log('Ex. 4.', peopleUnderAge25);
...
const sentencesWithMoreThan10Words = sentencesExercise5.filter(sentence => {
    // Split the sentence into words using space and count them will give us the sentences which has more than 10 words
    const wordCount = sentence.split(' ').length;
    return wordCount > 10;
});
console.log('Ex. 5.', sentencesWithMoreThan10Words);
...
// It did not work when the quantity is equal to 0, so instead I wrote quantity < 1
const outOfStockProducts = productsExercise6.filter(product => product.quantity < 1);
console.log('Ex. 6.', outOfStockProducts);
...
const numbersDivisibleByThree = numbersExercise7.filter(number => number % 3 === 0);
console.log('Ex. 7.', numbersDivisibleByThree);
...
const wordsWithMoreThan5Characters = wordsExercise8.filter(word => word.length > 5);
console.log('Ex. 8.', wordsWithMoreThan5Characters);
...
const lowGrades = studentsExercise9.filter(student => ["C-", "D+", "D", "D-", "F"].includes(student.grade));
console.log('Ex. 9', lowGrades);
...
const namesStartWithJ = usersExercise10.filter(user => user.name.startsWith('J'));
console.log('Ex. 10', namesStartWithJ);
